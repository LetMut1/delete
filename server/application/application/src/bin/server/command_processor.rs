...
use super::environment_configuration::Run as Run_;
...
static RUN_ENVIRONMENT_CONFIGURATION: OnceLock<EnvironmentConfiguration<Run_>> = OnceLock::new();
pub struct CommandProcessor<S> {
    _subject: PhantomData<S>,
}
pub struct Run;
impl CommandProcessor<Run> {
    pub fn process<'a>(environment_configuration_file_path: &'a str) -> Result<(), Error> {
        ...
        runtime.block_on(Server::run(environment_configuration))?;
        Ok(())
    }
    fn initialize_environment<'a>(environment_configuration_file_path: &'a str) -> Result<&'static EnvironmentConfiguration<Run_>, Error> {
        let environment_configuration = Loader::<Run>::load(environment_configuration_file_path)?;
        match RUN_ENVIRONMENT_CONFIGURATION.get() {
            Some(environment_configuration__) => Result::Ok(environment_configuration__),
            None => {
                if RUN_ENVIRONMENT_CONFIGURATION.set(environment_configuration).is_err() {
                    return Result::Err(
                        Error::new_(
                ...
                RUN_ENVIRONMENT_CONFIGURATION.get().into_value_does_not_exist(
                    Backtrace::new(
                        line!(),
                        file!(),
                    ),
    ...
    #[cfg(feature = "logging_to_file")]
    fn initialize_logging_to_fileger<'a>(environment_configuration: &'a EnvironmentConfiguration<Run_>) -> Result<WorkerGuard, Error> {
    ...
    fn initialize_runtime<'a>(environment_configuration: &'a EnvironmentConfiguration<Run_>) -> Result<Runtime, Error> {
