...
use super::raydium_amm_initialize_instruction_2::RaydiumAmmInitializeInstruction2;
...
impl TransactionParser {
    pub fn parse<'a>(environment_configuration: &'a EnvironmentConfiguration<ParseTransaction>) -> impl Future<Output = Result<(), Error>> + Send + Capture<&'a ()> {
        async move {
            let regular_expression = Regex::new(WorkflowData::<TransactionDifferentiation>::RAYDIUM_LIQUIDITY_POOL_V4_LOG_PATTERN).into_(
                Backtrace::new(
                    line!(),
                    file!(),
                ),
            ...
            let rpc_client = RpcClient::new_with_timeout(
                environment_configuration.subject.solana_cluster_url.clone(),
            ...
            'a: for solana_transaction_signature in environment_configuration.subject.solana_transaction_signature_registry.iter() {
                ...
                let compiled_instruction = &message.instructions[WorkflowData::<TransactionDifferentiation>::RAYDIUM_LIQUIDITY_POOL_V4_INSTRUCTION_VECTOR_INDEX];
                if is_right_transaction {
                    if message.account_keys[compiled_instruction.program_id_index as usize] != WorkflowData::<TransactionDifferentiation>::RAYDIUM_LIQUIDITY_POOL_V4_CONTRACT_PUBKEY {
                        is_right_transaction = false;
                    }
                }
                if is_right_transaction {
                    match ui_transaction_status_meta.inner_instructions {
                        OptionSerializer::Some(ui_inner_instruction_registry) => {
                            ...
                            && ui_inner_instruction_registry[0].index as usize == WorkflowData::<TransactionDifferentiation>::RAYDIUM_LIQUIDITY_POOL_V4_INSTRUCTION_VECTOR_INDEX
                            && ui_inner_instruction_registry[0].instructions.len() == WorkflowData::<TransactionDifferentiation>::RAYDIUM_LIQUIDITY_POOL_V4_INNER_INSTRUCTIONS_QUANTITY;
                        }
                        _ => {
                            is_right_transaction = false;
                        }
                ...
                if is_right_transaction {
                    match ui_transaction_status_meta.log_messages {
                        OptionSerializer::Some(log_message_registry) => {
                            is_right_transaction = regular_expression.is_match(
                                log_message_registry[WorkflowData::<TransactionDifferentiation>::RAYDIUM_LIQUIDITY_POOL_V4_LOG_VECTOR_INDEX].as_str()
                            );
                        }
                        _ => {
                            is_right_transaction = false;
